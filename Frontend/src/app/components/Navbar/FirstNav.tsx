...
import DesktopFirstNav from "./component/DesktopFirstNav"

const FirstNav = () => {
  const [nav, setNav] = useState(false)
  const [openDropdownId, setOpenDropdownId] = useState(null)
  let role = useAppSelector((state: any) => state.auth.role)
  const handleNav = () => {
    setNav(!nav)
  }
  ...
  return (
    <>
      <div className="bg-slate-100">
          ...
          {logItems.map(item => (
            <div key={item.id} className="relative mr-10">
              ...
              {openDropdownId === item.id && (
                <ul
                  tabIndex={0}
                  className="absolute dropdown-content menu bg-base-100 rounded-box z-[1] w-40 p-2 shadow"
                >
        ...
        {/* Desktop Navigation */}
        <DesktopFirstNav navItems={navItems} role={role} />
